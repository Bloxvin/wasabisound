...
s3 = boto3.resource('s3',
                  ...
                  aws_access_key_id="ATYUVBHYFCRSUDN7RCOJ",
                  aws_secret_access_key="a0rCrsqR9VdMKRejpoM8SwF8wiZ4Hmfj1NUn6tmz")

my_bucket = s3.Bucket('wasabisound')
songs_folder = os.getcwd()+"/Cloud Songs/"

if not os.path.exists(songs_folder):
    os.mkdir(songs_folder)

def update_cloud():
    print("Updating Cloud...", end="")
    for my_bucket_object in my_bucket.objects.all():
        # print(my_bucket.objects.all())
        # print(my_bucket_object)
        song_file = songs_folder + my_bucket_object.key

        # How to get file_type of an object in wasabi cloud?
        if not os.path.exists(song_file) and os.path.splitext(song_file)[1] == ".mp3":
            my_bucket.download_file(my_bucket_object.key, song_file)
            print("Downloaded")

    # for song in os.listdir(songs_folder):
    #     print(song)
    #     print(my_bucket.objects.all())
    #     if os.path.splitext(song)[1] == ".mp3" and not song in my_bucket.objects.all():
    #         my_bucket.upload_file(songs_folder + song, song)
    #         print("Uploaded")

    print("Successful!")
    # root.after(10000, update_cloud())
...
def sync_music_player():
    load(playlist)
    update_cloud()
    root.after(10000, sync_music_player)
...
def play_song(song_name: StringVar, song_list: Listbox, status: StringVar):
    ...
    print(song_list.get(ACTIVE))
    name = song_list.get(ACTIVE)
    song_file = songs_folder+song_list.get(ACTIVE)
    if len(name) > 40:
        name = (name[:35] + '.mp3')
    song_name.set(name)
    ...
    metadata = audio_metadata.load(song_file)
...
def load(listbox):
    ...
    # for i in listbox:
    #     listbox.remove(END, i)
    tracks = os.listdir(songs_folder)

    # Takes all the songs from the directory and store in listbox.
    print(listbox.size())
    listbox.delete(0, listbox.size())
    for track in tracks:
        # print(os.path.splitext(track)[1])
        # print(os.path.exists(songs_folder+track))
...
root.configure(bg='#65A8E1')
root.title('Wasabi KKK Music Player')
...
upload_btn = Button(button_frame, text="Sync local folder with cloud", bg='Aqua', font=('Georgia', 13), width=25,
                   command=lambda: upload_to_cloud())
upload_btn.place(x=50, y=60)

# dir_btn = Button(button_frame, text="Load Directory", bg='Aqua', font=("Georgia", 13), width=35,
#                  command=lambda: load(playlist))
# dir_btn.place(x=10, y=75)

# Control the volume of the song.
volume_slider = Scale(volume_frame, from_=100, to=0, orient=VERTICAL, command=volume, length=110, bg='orange',
                      cursor='hand2')
